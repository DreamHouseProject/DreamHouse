...
module.exports = function (app) {
    ...
    app.get("/api/search", function (req, res) {
       ...
       request(queryURL, function(err, response){
        ...
        for (i=0;i<10;i++){
            list.push({
                homeAddress: response.body.bundle[i].UnparsedAddress,
    ...
    app.get("/api/searchOnboard", function(req,res){
        ...
        var queryURL = "https://search.onboard-apis.com/communityapi/v2.0.0/Area/Full/?AreaId=ZI" + ACCESS_TOKEN + "&near=" + location + "&limit=100";
        request(queryURL, function (err, response) {
            response.body = JSON.parse(response.body)
            //    console.log(response.body)
